...
class TestValidations(ValidationsBaseTest):
    ...
    def test_user_enter_an_invalid_email2(self):
        response = self.client.post("api/v2/auth/signup", data = json.dumps(self.user_email2_invalid), content_type = "application/json")
        self.assertEqual(response.status_code, 400)
        result = json.loads(response.data.decode('utf-8'))
        self.assertEqual(result['error'],"Email is Invalid")
    ...
    def test_correct_user_pasword_length(self):
        response = self.client.post("api/v2/auth/signup", data = json.dumps(self.user_password_length), content_type = "application/json")
        self.assertEqual(response.status_code, 400)
        result = json.loads(response.data.decode('utf-8'))
    ...
    def test_user_pasword_is_alphabets(self):
        response = self.client.post("api/v2/auth/signup", data = json.dumps(self.user_password_alphabetic), content_type = "application/json")
        self.assertEqual(response.status_code, 400)
        result = json.loads(response.data.decode('utf-8'))
    ...
    def test_user_pasword_contains_capital(self):
        response = self.client.post("api/v2/auth/signup", data = json.dumps(self.user_password_capital), content_type = "application/json")
        self.assertEqual(response.status_code, 400)
        result = json.loads(response.data.decode('utf-8'))
    ...
    def test_user_pasword_number(self):
        response = self.client.post("api/v2/auth/signup", data = json.dumps(self.user_password_number), content_type = "application/json")
        self.assertEqual(response.status_code, 400)
        result = json.loads(response.data.decode('utf-8'))
